...
std::string fileNameRoot = "book_csma";    // base name for trace files, etc
...
TraceCwnd ()    // Trace changes to the congestion window
{
  ...
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream (outputNameRoot + ".cwnd");
...
int main (int argc, char *argv[])
{
  ...
  CsmaHelper csmaAR, csmaRB;


  // create point-to-point link from A to R
  csmaAR.SetChannelAttribute("DataRate", StringValue("10Mbps"));
  csmaAR.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (delayAR)));
  csmaAR.SetQueue("ns3::DropTailQueue", "MaxPackets", UintegerValue(queuesize));
  devAR = csmaAR.Install(NodeContainer(A,R));

  // create point-to-point link from R to B
  csmaRB.SetChannelAttribute("DataRate", StringValue("1Mbps"));
  csmaRB.SetChannelAttribute ("Delay", TimeValue (MilliSeconds (delayAR)));
  csmaRB.SetQueue("ns3::DropTailQueue", "MaxPackets", UintegerValue(queuesize));
  devRB = csmaRB.Install(NodeContainer(R,B));
  ...
  // Set up tracing
  AsciiTraceHelper ascii;
  std::string tfname = outputNameRoot + ".tr";
  csmaAR.EnableAsciiAll (ascii.CreateFileStream (tfname));
  ...
  csmaAR.EnablePcapAll (outputNameRoot);    // ".pcap" suffix is added automatically
